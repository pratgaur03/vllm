...
def run_vllm(
...
) -> tuple[float, Optional[list[RequestOutput]], dict]:
    ...
    for output in outputs or []:
        ...
        prompt_len = len(output.prompt_token_ids or [])
        ...
        print(f"  Decode latency: {decode_latency:.4f}s for {decode_len} tokens")
        print(f"  Decode iters: {decode_iters}")

        total_prefill_time += prefill_latency
        total_decode_time += decode_latency
        total_prefill_tokens += prompt_len
        total_decode_tokens += decode_len

    if total_prefill_time > 0:
        print(f"Prefill throughput: {total_prefill_tokens / total_prefill_time:.2f} tokens/s")
    if total_decode_time > 0:
        print(f"Decode throughput: {total_decode_tokens / total_decode_time:.2f} tokens/s")
...
def main(args: argparse.Namespace):
    ...
    if args.backend == "vllm":
        if args.async_engine:
            ...
            elapsed_time, request_outputs, extra_metrics  = run_vllm(
    ...
    if args.output_json:
        ...
        if extra_metrics:
            if extra_metrics["prefill_time"] > 0:
                results["prefill_throughput"] = (
                    extra_metrics["prefill_tokens"] / extra_metrics["prefill_time"])
            if extra_metrics["decode_time"] > 0:
                results["decode_throughput"] = (
                    extra_metrics["decode_tokens"] / extra_metrics["decode_time"])
            results["prefill_tokens"] = extra_metrics["prefill_tokens"]
